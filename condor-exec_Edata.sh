...
for nEdg in 1 2 3
do
	for cen in "c_pr" "c_nxkatz" "c_nxeigen" "c_nxpr"
	do
		for isDir in 0 1
		do
			python3 centralityProfileEdataRun.py -f test.txt -o data100e_N${N}_${cen}_nEdg${nEdg}_Dir${isDir}.pkl --Ntr=${N} -nE=${nEdg} -Dir=${isDir} -c ${cen}
		done
	done
done
